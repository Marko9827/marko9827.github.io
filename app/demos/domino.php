...
if (!empty($_GET['action'])) {
    ...
    $code = $_GET['code'] ?? '';
    $role = $_GET['role'] ?? '';
    $response = ['moves' => [], 'chats' => [], 'turn' => null];

    switch ($action) {
        case 'join':
            // ako soba ne postoji, kreiraj je
            if (!isset($_SESSION['rooms'][$code])) {
                $_SESSION['rooms'][$code] = [
                    'moves' => [],
                    'chats' => [],
                    'turn' => 0,
            ...
            $response['turn'] = $_SESSION['rooms'][$code]['turn'];
            break;

        case 'move':
            // primljeni tile kao "a,b"
            $tile = $_GET['tile'] ?? '';
            $useLeft = isset($_GET['useLeft']) ? (bool) $_GET['useLeft'] : false;
            if (isset($_SESSION['rooms'][$code])) {
                $_SESSION['rooms'][$code]['moves'][] = [
                    'tile' => $tile,
                    'useLeft' => $useLeft,
                ];
                // promeni red
                $_SESSION['rooms'][$code]['turn'] = 1 - $_SESSION['rooms'][$code]['turn'];
            }
            break;

        case 'draw':
            // sifra samo okida redosled
            if (isset($_SESSION['rooms'][$code])) {
                $_SESSION['rooms'][$code]['turn'] = 1 - $_SESSION['rooms'][$code]['turn'];
            }
            break;
        ...
        case 'chat':
            $text = $_GET['text'] ?? '';
            if (isset($_SESSION['rooms'][$code])) {
                $_SESSION['rooms'][$code]['chats'][] = [
                    'from' => $role === 'host' ? 'Igrač 1' : 'Igrač 2',
                    'text' => $text,
                ];
            }
            break;
        ...
        case 'poll':
            if (isset($_SESSION['rooms'][$code])) {
                $room = &$_SESSION['rooms'][$code];
                // isporuči i očisti
                $response['moves'] = $room['moves'];
                $response['chats'] = $room['chats'];
