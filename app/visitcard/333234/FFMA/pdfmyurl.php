 ...
 *		$pdf->SetPdfCreatorVersion(...);			// set the PDF creator version that needs to be used for the conversion
 ...
 *      $pdf->SetDimensionUnit(....);				// Set the unit of measure for the dimensions (mm, cm, inches or pixels)
 ...
 *      $pdf->SetCssMediaType(...);					// Define the CSS media type that should be used for the conversion, you can use this for the print layout
 *      $pdf->OptimizeLayout(...);					// Optimize fonts and colors in the PDF and make sure static items only occur once
 *      $pdf->LazyLoad(...);						// Load the page "lazily" so that all content appears as if you've scrolled down the page before grabbing the PDF
...
class PDFmyURL {
	...
	// Allows you to set the pdf creator version
	// Currently the following versions are supported: 1.7, 2.0 and 3.0
	public function SetPdfCreatorVersion($version) {
		$this->options['pdf_creator_version'] = $version;
	}
	
	// Allows you to activate debugging on the PDFmyURL servers for your conversions when support asks for it
	public function ActivateDebugging() {
	...
	//
	// By default the viewport is 1366*768
	public function SetViewport($width = 1366, $height = 768) {
	...
	// Wait for an extra time after page load (for example for redirections), but only if it's greater than 0
	//
	// Returns true on success and false if the passed value is not correct
	public function SetWaitTime($msec) {
		$ms = intval($msec);
		if ($ms > 0) {
			$this->options['wait_time'] = $ms;
			return true;
		} else
			return false;
	}
	
	// Optimizes the layout to correct certain screen features (such as the static navigation bar) or colors & fonts
	//
	public function OptimizeLayout() {
		$this->options['optimize_layout'] = 'true';
	}
	
	// Activates lazy loading of all content, useful for single page apps and pages that have content loaded via Ajaz
	//
	public function LazyLoad() {
		$this->options['lazy_load'] = 'true';
	}
	...
	public function HideContent($divs) {
		if (is_array($divs)) {
			...
			$this->options['content'][$divs] = 'hide';
		}
	...
	public function ShowContent($divs) {
		if (is_array($divs)) {
			...
			$this->options['content'][$divs] = 'show';
		}
	}
